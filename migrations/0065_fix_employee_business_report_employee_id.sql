...
BEGIN
  ...
  WITH authorized_orders AS (
    ...
    SELECT ledger.*, COALESCE(owner.id, NULLIF(ledger.employee_id, ''), NULLIF(reversed_source.employee_id, ''), 'unassigned') AS report_employee_id,
      COALESCE(owner.display_name, NULLIF(ledger.employee_id, ''), NULLIF(reversed_source.employee_id, ''), 'Chưa phân công') AS report_employee_name
...
VALUES ('0065', 'Fix employee business report ledger employee identity ambiguity')
