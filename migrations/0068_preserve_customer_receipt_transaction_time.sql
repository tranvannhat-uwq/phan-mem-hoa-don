...
-- Preserve the selected Vietnam business time for customer receipts.  The
-- receipt, cashbook and debt ledger must use one timestamp; audit posting time
-- remains now() and no historical row is changed.

DROP FUNCTION IF EXISTS public.rpc_record_customer_receipt(text, numeric, text, text, text, text);
...
DECLARE
  ...
  receipt_date timestamptz := COALESCE(p_transaction_date, now());
BEGIN
  ...
  request_hash := md5(jsonb_build_object(
    ...
    'category', resolved_category, 'transactionDate', receipt_date
  )::text);
  ...
  INSERT INTO public.cashbook_transactions(
    ...
    cashbook_id, receipt_date, receipt_date, 'thu', 'customer_payment', 'in', resolved_category,
  ...
  INSERT INTO public.customer_debt_transactions(
    ...
    actor.auth_user_id::text, receipt_date
  );

  UPDATE public.customers
  SET debt = new_balance,
      last_payment_at = GREATEST(COALESCE(last_payment_at, '-infinity'::timestamptz), receipt_date),
      updated_at = now(),
...
-- 6-parameter compatibility wrapper used by earlier category-aware clients.
CREATE OR REPLACE FUNCTION public.rpc_record_customer_receipt(
  p_customer_id text,
  p_amount numeric,
  p_notes text,
...
BEGIN
  RETURN public.rpc_record_customer_receipt(
    p_customer_id, p_amount, p_notes, p_payment_method, p_idempotency_key, p_category, NULL
  );
END;
$$;
...
BEGIN
  RETURN public.rpc_record_customer_receipt(
    p_customer_id, p_amount, p_notes, p_payment_method, p_idempotency_key, 'Thu tiền khách hàng', NULL
  );
END;
$$;

REVOKE ALL ON FUNCTION public.rpc_record_customer_receipt(text, numeric, text, text, text, text, timestamptz) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.rpc_record_customer_receipt(text, numeric, text, text, text, text, timestamptz) TO authenticated;
...
VALUES ('0068', 'Preserve selected transaction time for customer receipts')
