...
-- Allow privileged users (admin and accounting) to adjust the order business date (order_date)
-- when amending an existing finalized order or updating a draft order.
-- Non-privileged users (sales) cannot adjust historical order dates.
...
BEGIN
  actor := public.get_authenticated_profile();
...
BEGIN
  ...
  target_order_date := original_order.order_date;
  IF NULLIF(btrim(p_order->>'date'), '') IS NOT NULL THEN
    target_order_date := (p_order->>'date')::timestamptz;
    IF (target_order_date AT TIME ZONE 'Asia/Bangkok')::date > (now() AT TIME ZONE 'Asia/Bangkok')::date THEN
      RAISE EXCEPTION 'Order date cannot be in the future';
    END IF;
    IF target_order_date > now() THEN
      target_order_date := now();
    END IF;
  END IF;
  ...
  -- Neutralize the commission currently attributable to this order.
  ...
  -- Update customer debt transactions and balance. Align transaction_date with target_order_date.
    ...
    IF customer_id IS NOT NULL THEN
      ...
      SET debt = new_balance,
          ...
          net_revenue = COALESCE(old_customer.net_revenue, 0)
            - COALESCE(original_order.total_payable, 0) + total_payable,
          last_order_at = GREATEST(COALESCE(old_customer.last_order_at, target_order_date), target_order_date),
      ...
      IF round(debt_delta) <> 0 THEN
        ...
        INSERT INTO public.customer_debt_transactions(
          ...
          actor.auth_user_id::text, target_order_date
        );
      END IF;
    END IF;
  ELSE
    ...
    IF customer_id IS NOT NULL THEN
      ...
      SET debt = new_balance,
          ...
          last_order_at = GREATEST(COALESCE(customer_row.last_order_at, target_order_date), target_order_date),
          updated_at = now(), updated_by = actor.auth_user_id::text
      WHERE customer.id = customer_id;
  ...
  -- Align existing customer debt transactions if order date was modified
  IF target_order_date IS DISTINCT FROM original_order.order_date THEN
    UPDATE public.customer_debt_transactions ledger
    SET transaction_date = target_order_date
    WHERE ledger.order_id = original_order.id
      AND ledger.transaction_type IN ('order', 'order_amend')
      AND ledger.transaction_date IS DISTINCT FROM target_order_date;
  END IF;

  SELECT * INTO STRICT amended_order
  FROM public.orders sale WHERE sale.id = original_order.id;
  IF amended_order.order_date IS DISTINCT FROM target_order_date
...
VALUES ('0059', 'Allow privileged order date amendment and draft order timestamp edit')
