...
import { dbSaveOrder, dbCreateQuickCustomer, dbConfirmOrder, dbAmendOrder, dbFetchOrderDebtSnapshot, dbLoadCustomerAssignedPricing, dbRefreshCustomerFinancialState, dbRefreshOrderById, cacheOrdersLocally, isCloudActive } from '../services/supabase.js?v=20260916-accounting-product-actions-v1';
import { renderAll, switchTab } from '../main.js?v=20260916-accounting-product-actions-v1';
import { populatePricelistsDropdowns } from './pricelists.js?v=20260916-accounting-product-actions-v1';
import { generateUniqueCustomerCode } from './customers.js?v=20260916-accounting-product-actions-v1';
import { addCashbookTransaction } from './so_quy.js?v=20260916-accounting-product-actions-v1';
import { getApplicablePriceList, resolveCustomerProductPrice, normalizePriceListType, PRICE_LIST_TYPES, filterPriceListsForUser, canUserViewPriceList, canUserUsePriceListForCustomer, isDealerPrivatePriceList, isUsableResolvedPrice, shouldOverrideWithGlobalCustomerPriceList } from '../domain/pricing.js?v=20260916-accounting-product-actions-v1';
import { normalizeCustomerPhone } from '../domain/customer-query.js';
import { isPrintOnlyPriceList, parseInvoicePercent, requiresOrderSaveApproval, sanitizeInvoicePercentInput, supportsInvoiceLineDiscount } from '../domain/invoice-discount.js?v=20260916-accounting-product-actions-v1';
...
import { chargeCustomerDebt, getOrderDebtSnapshot, getOrderOutstandingAmount } from '../domain/customer-debt.js?v=20260916-accounting-product-actions-v1';
import { getOrderDisplayCode } from '../domain/order-display.js';
import { canAdjustOrderBusinessDate, currentBusinessDateTimeInputValue, resolveOrderBusinessDateTimeForSave } from '../domain/order-business-date.js?v=20260916-accounting-product-actions-v1';
...
export function compileActiveOrder(customerOverride = null) {
  ...
  const originalOrderDate = saveBtn ? saveBtn.getAttribute('data-original-order-date') : null;
  ...
  if (originalOrderDate && !Number.isNaN(new Date(originalOrderDate).getTime())
...
export async function saveActiveOrder(status = 'settled') {
  ...
  try {
    ...
    if (amendOrderId && status !== 'settled') {
      showToast('Đơn đã chốt chỉ có thể được cập nhật ở trạng thái đã chốt.', 'warning');
      return null;
    }
    ...
    if (saved) {
      ...
      if (amendOrderId) {
        ...
        resetInvoiceBuilder();
        renderAll();
        showToast(`Đã cập nhật trực tiếp đơn ${persistedOrder.id}, gồm cả ngày giờ lên đơn.`, 'success');
