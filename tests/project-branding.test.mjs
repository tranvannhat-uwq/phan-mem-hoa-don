...
test('SoVie is the primary project brand in navigation and login', () => {
  ...
  assert.match(html, /class="login-showcase-logo"[\s\S]*alt="SoVie"/);
...
test('project logo presentation is prominent, transparent and responsive', () => {
  const css = read('style.css');

  assert.match(css, /\.brand-project-logo\s*\{[\s\S]*background:\s*transparent\s*!important;[\s\S]*box-shadow:\s*none\s*!important;/);
  assert.match(css, /\.login-brand-lockup\s*\{[\s\S]*width:\s*min\(320px, 92%\);[\s\S]*box-shadow:\s*none\s*!important;/);
  assert.match(css, /@media \(max-width: 768px\)[\s\S]*\.brand-project-logo\s*\{[\s\S]*width:\s*138px\s*!important;/);
